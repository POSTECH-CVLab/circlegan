...
def load_model(saver, sess, checkpoint_dir, ckpt_number=0):
    ...
    if ckpt and ckpt.model_checkpoint_path:
        if ckpt_number == 0:
            ...
            ckpt_to_restore = checkpoint_dir+'/model-%i' % ckpt_number
...
def save_images(images, size, path):
    ...
    merged_image = (merged_image + 1.0) / 2 * 255.0
    return Image.fromarray(merged_image.astype('uint8')).save(path)


def imread(path, is_grayscale=False):
    if (is_grayscale):
        return Image.open(path).convert('L').astype(np.float)
    else:
        return Image.open(path).astype(np.float)
...
def center_crop(x, crop_h, crop_w=None, resize_w=64):
    ...
    return x.resize([resize_w, resize_w], Image.BILINEAR)
...
def batch_resize(images, newHeight, newWidth):
    ...
    for idx, image in enumerate(images):
        ...
        images_resized[idx] = image.resize([newHeight, newWidth], Image.BILINEAR)
